...
def main():
    ...
    measurement_files = st.sidebar.file_uploader(
        "Measurement Files", accept_multiple_files=True
    )
        ...
        if section_key != "params":
            ...
            for item in section["items"]:
                ...
                if item["type"] == "number":
                    ...
                    config[section_key][item["name"]] = st_.selectbox(
                        ...
                        index=item["options"].index(config[section_key][item["name"]]),
                        help=item["help"],
                    )
            ...
            for param in config["params"]:
                ...
                st_.subheader(
                    param["name"]
                    + "("
                    + (param["unit"] if "unit" in param else "-")
                    + ")"
                )
    ...
    func = fit.define_func(
        config["params"], config["func"]["defs"], config["func"]["expr"]
    )
    ...
    if measurement_files:
        ...
        if st.button("Fit!", help="Do Fitting!"):
            ...
            for measurement_file, dir_name in zip(measurement_files, dir_names):
                ...
                for f, z in zip(freq, z_measured):
                    if (
                        config["fitting"]["lower frequency"]
                        <= float(f)
            ...
            # Show last data
            fit.show_data(
                freq, z_measured, z_calc, param_names, param_values, param_units, loss
            )
        ...
        freq = fit.get_freq(
            config["fitting"]["lower frequency"], config["fitting"]["upper frequency"]
        )
